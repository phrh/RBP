...
public class Trial extends Thread {
	...
	private void execute()
	{
		/********** if (this.stop_thread) { break; }*/
			...
			//System.out.println("Start trial " + this.tr);
			...
			for (int cnt=0; cnt<iter; cnt++)
			{
				...
				{
					System.out.println("Iteration " + cnt + " Trial " + tr);
				}
					//printToScreen(tf, tr, cnt, bestZ, bestC, bestlogl, -1, -1); TODO
				*/
				if((cnt+1)%outputStep == 0)
				{
				...
				if (logl[cnt] > bestlogl) 
				{
					...
					for (int i=0; i<Z.length; i++) bestZ[i] = Z[i];
					
					for (int i=0; i<C.length; i++) bestC[i] = C[i];
				}
